...
template <typename Dtype>
void caffe_gpu_gemm(cublasHandle_t cublas_handle, const CBLAS_TRANSPOSE TransA,
    const CBLAS_TRANSPOSE TransB, const int M, const int N, const int K,
    const Dtype alpha, const Dtype* A, const Dtype* B, const Dtype beta,
    Dtype* C);
...
void caffe_gpu_gemm(const CBLAS_TRANSPOSE TransA,
    ...
    const Dtype alpha, const Dtype* A, const Dtype* B, const Dtype beta,
    Dtype* C) {
  caffe_gpu_gemm(Caffe::cublas_handle(), TransA, TransB, M, N, K,
      alpha, A, B, beta, C);
}
...
template <typename Dtype>
void caffe_gpu_axpy(cublasHandle_t cublas_handle, const int N,
    const Dtype alpha, const Dtype* X, Dtype* Y);

template <typename Dtype>
void caffe_gpu_axpby(cublasHandle_t cublas_handle, const int N,
...
template <typename Dtype>
void caffe_gpu_scal(cublasHandle_t cublas_handle,
...
void caffe_gpu_rng_gaussian(
    const int n, const Dtype mu, const Dtype sigma, Dtype* r);

template <typename Dtype>
void caffe_gpu_rng_bernoulli(const int n, const Dtype p, int* r);

template <typename Dtype>
Dtype caffe_gpu_dot(const int n, const Dtype* x, const Dtype* y);
...
template <typename Dtype>
uint32_t caffe_gpu_hamming_distance(
    const int n, const Dtype* x, const Dtype* y);

template <typename Dtype>
Dtype caffe_gpu_asum(cublasHandle_t cublas_handle, const int n, const Dtype* x);

template <typename Dtype>
Dtype caffe_gpu_asum(const int n, const Dtype* x) {
  return caffe_gpu_asum(Caffe::cublas_handle(), n, x);
}
